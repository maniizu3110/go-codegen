...
const commonRepositoryFileName = "common.go"

//modeasl はfilesに含まれるので削除する予定
func Gen(setting util.RepositorySetting, files []fs.FileInfo) {
	if _, err := os.Stat(setting.OutputDir); os.IsNotExist(err) {
		if err := os.MkdirAll(setting.OutputDir, 0777); err != nil {
			fmt.Println(err)
		}
	}
	if !IsCommonFileExist(setting.OutputDir, commonRepositoryFileName) {
		createCommonRepositoryFile(setting)
	}
	for i := range files {
...
func Create(setting util.RepositorySetting, fileName string) {
